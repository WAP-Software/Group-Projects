...
const COLUMNS: Array<"backlog" | "in_progress" | "review" | "done"> = [
...
export function useTasks(workspaceId: string) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (data) {
        ...
        data.forEach((t) => {
          if (grouped[t.status as keyof TasksByStatus]) {
            grouped[t.status as keyof TasksByStatus].push(t);
          }
        });
        setTasks(grouped);
      }
  ...
  async function moveTask(
    ...
    updated[task.status as keyof TasksByStatus] = updated[task.status as keyof TasksByStatus].filter(
      (t) => t.id !== taskId,
    );
    const moved = { ...task, status: newStatus, position: newPosition };
    updated[newStatus] = [
      ...updated[newStatus].slice(0, newPosition),
      moved,
      ...updated[newStatus].slice(newPosition),
    ];
